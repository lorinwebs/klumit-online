    ...
    >
      ...
      <nav className="w-full px-6 md:px-12 lg:px-16 py-4 md:py-5 grid grid-cols-[auto_1fr_auto] items-center gap-6 md:gap-12">
        ...
        <div className="flex items-center order-1 md:order-1">
          ...
          <Link
            ...
            className="md:hidden text-xs font-light tracking-[0.2em] uppercase text-[#1a1a1a] hover:opacity-60 transition-opacity whitespace-nowrap relative z-10 flex items-center gap-1.5"
            aria-label={t('header.talkToUs')}
            onClick={(e) => e.stopPropagation()}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="#25D366" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
        ...
        <div className="flex items-center justify-center order-2">
          ...
          <nav className="hidden md:flex items-center justify-center gap-10 lg:gap-12" aria-label="תפריט ניווט ראשי" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
            <Link href="/products?tab=bags" className={`text-sm tracking-[0.25em] uppercase transition-opacity duration-300 whitespace-nowrap ${isProductsPage && currentTab === 'bags' ? 'text-[#1a1a1a] opacity-100' : 'text-[#1a1a1a] opacity-60 hover:opacity-100'}`} style={{ fontWeight: 100 }}>
              {t('header.bags')}
            </Link>
            <span className="w-px h-3 bg-black/10 shrink-0" />
            <Link href="/products?tab=belts" className={`text-sm tracking-[0.25em] uppercase transition-opacity duration-300 whitespace-nowrap ${isProductsPage && currentTab === 'belts' ? 'text-[#1a1a1a] opacity-100' : 'text-[#1a1a1a] opacity-60 hover:opacity-100'}`} style={{ fontWeight: 100 }}>
              {t('header.belts')}
            </Link>
            <span className="w-px h-3 bg-black/10 shrink-0" />
            <Link href="/products?tab=wallets" className={`text-sm tracking-[0.25em] uppercase transition-opacity duration-300 whitespace-nowrap ${isProductsPage && currentTab === 'wallets' ? 'text-[#1a1a1a] opacity-100' : 'text-[#1a1a1a] opacity-60 hover:opacity-100'}`} style={{ fontWeight: 100 }}>
              {t('header.wallets')}
            </Link>
          </nav>
        ...
        <div className="flex items-center gap-4 md:gap-6 shrink-0 order-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center justify-center w-7 h-7 md:w-8 md:h-8 hover:opacity-60 transition-opacity duration-300"
              aria-label="בחר שפה"
            >
      ...
      <div className="md:hidden w-full flex items-center justify-center gap-6 h-10 border-t border-black/5 bg-white/95 backdrop-blur-sm" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
        <Link 
            ...
            className={`flex items-center h-full text-xs tracking-[0.2em] uppercase transition-opacity duration-300 ${isProductsPage && currentTab === 'bags' ? 'text-[#1a1a1a] opacity-100' : 'text-[#1a1a1a] opacity-60 hover:opacity-100'}`}
        ...
        <Link 
            ...
            className={`flex items-center h-full text-xs tracking-[0.2em] uppercase transition-opacity duration-300 ${isProductsPage && currentTab === 'belts' ? 'text-[#1a1a1a] opacity-100' : 'text-[#1a1a1a] opacity-60 hover:opacity-100'}`}
            style={{ fontWeight: 100 }}
        >
            {t('header.belts')}
        </Link>
        ...
        <Link 
            ...
            className={`flex items-center h-full text-xs tracking-[0.2em] uppercase transition-opacity duration-300 ${isProductsPage && currentTab === 'wallets' ? 'text-[#1a1a1a] opacity-100' : 'text-[#1a1a1a] opacity-60 hover:opacity-100'}`}
            style={{ fontWeight: 100 }}
        >
            {t('header.wallets')}
        </Link>
